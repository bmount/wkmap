bicsymaps.ukbbuf = function() {
  var ukbbuf = {},
      view,
      url,
      zoom = Math.round;

  ukbbuf.view = function(x) {
    if (!arguments.length) return view;
    view = x;
    return ukbbuf;
  };

  ukbbuf.url = function(x) {
    if (!arguments.length) return url;
    url = typeof x === "string" && /{.}/.test(x) ? _url(x) : x;
    return ukbbuf;
  };

  ukbbuf.zoom = function(x) {
    if (!arguments.length) return zoom;
    zoom = typeof x === "function" ? x : function() { return x; };
    return ukbbuf;
  };

  ukbbuf.render = function(canvas, callback) {
    ...
    tiles.forEach(function(tile) {
      ...
      return key == null ? done() : bicsymaps_cache(key, function(mapdata) {
        if (!mapdata) return;
        //if (mapdata instanceof ArrayBuffer) {
          var tmpctx = document.createElement("canvas")
          tmpctx.height = 255;
          tmpctx.width = 255;
          var renderedGeoms = uncollect(mapdata, tmpctx).toDataURL()
            , gimg = new Image();
            gimg.src = renderedGeoms;
          context.drawImage(gimg, dx * (tile[0] - x0), dy * (tile[1] - y0));
          done();
        //}
        //else {
        //  context.drawImage(mapdata, dx * (tile[0] - x0), dy * (tile[1] - y0));
        //  done();
        //}
      });
